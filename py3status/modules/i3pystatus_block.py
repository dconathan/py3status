...
The modules parameters are provide as such
```
i3pystatus_block clock {
    module = 'clock'
...
MIN_CHECK_INTERVAL = 5  # the minimum time between checking the modules output
...
class Py3status:
    ...
    def post_config_hook(self):
        ...
        # interval.
        self._cache_timeout = min(getattr(module, 'interval', None),
                                  MIN_CHECK_INTERVAL)
    ...
    def run(self):
        ...
        if self.is_interval_module:
            output['cached_until'] = self.py3.time_in(
                sync_to=self._cache_timeout
            )
        else:
            output['cached_until'] = self.py3.CACHE_FOREVER
